...
namespace :piggy do
  ...
  task :update => :configuration do
    ...
    $config['currencies'].each do |curr|
      ...
      cmdout = `curl "http://www.bloomberg.com/markets/chart/data/1Y/#{curr.upcase}USD:CUR" -H 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10.9; rv:31.0) Gecko/20100101 Firefox/31.0' -H 'X-Requested-With: XMLHttpRequest' -H "Referer: http://www.bloomberg.com/quote/#{curr.upcase}USD:CUR"`
